...
const UnassignedRfidsStatus = () => {
  const [inactiveRfids, setInactiveRfids] = useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const inactiveRfidsCount = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/rfidkey`
        );

        const inactiveRfids = inactiveRfidsCount.data.filter(
          (rfid) => rfid.rfidStatus === "UNASSIGNED"
        );

        setInactiveRfids(inactiveRfids.length);
      } catch (err) {
        if (err.response) {
          const errorMessage =
      ...
      <div className="allStastics">
        <div className="Stastics-card red-card-outline">
          <div className="Stastics-card-details">
            ...
            <p className="Stastics-text-title textcenter">Inactive Rfids</p>

            <p className="Stastics-text-body textcenter">{inactiveRfids}</p>
          </div>
...
export default UnassignedRfidsStatus;
